...
#   C5 (T-020-04) — a client repository installs the packed @kippu/api and
#                   compiles typed calls against it;
#   C6 (T-026-01) — the same client installs the packed @kippu/metadata-schema
...
pack @kippu/api api
pack @kippu/metadata-schema metadata-schema
api_tarball="$(ls "$work"/kippu-api-*.tgz)"
schema_tarball="$(ls "$work"/kippu-metadata-schema-*.tgz)"
...
# @kippu/api is types only: no implementation may ship in it.
...
node -e '
  ...
  manifest.dependencies["@kippu/metadata-schema"] = `file:${process.argv[2]}`;
  fs.writeFileSync("package.json", JSON.stringify(manifest, null, 2));
' "$api_tarball" "$schema_tarball"

pnpm install --ignore-workspace --no-frozen-lockfile
pnpm run typecheck
echo "The client fixture compiles against the packed @kippu/api and @kippu/metadata-schema."

node --input-type=module -e '
  import assert from "node:assert/strict";
  import { EVENT_SCHEMA_ID, CLASS_SCHEMA_ID, eventSchema, classSchema, lintPersonalData } from "@kippu/metadata-schema";
  import eventFile from "@kippu/metadata-schema/event/1.0.json" with { type: "json" };
  import classFile from "@kippu/metadata-schema/class/1.0.json" with { type: "json" };
  ...
  console.log("The client fixture reads the schemas from the packed @kippu/metadata-schema.");
'
