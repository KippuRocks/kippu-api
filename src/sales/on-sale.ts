...
import { salesClosed } from "./allocation.js";
import { CheckoutError } from "./ports.js";
 ...
 * issues nothing, so nothing of it is sold or held. With `db`, an event whose
 * sales Kippu has closed for a seal or cancellation in progress is refused too
 * (`CheckoutError("sales-closed")`, `T-022-05`).
 */
export async function eventOnSale(
  ...
  if (db !== undefined && (await salesClosed(db, event))) {
    throw new CheckoutError(
      "sales-closed",
      "the event's sales are closed: it is being sealed or cancelled",
    );
  }
  return found.value;
}
