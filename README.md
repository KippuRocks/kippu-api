...
- **Seat positions** (`events.zones.addSeatPositions`, `events.zones.seatPositions`) are each seated zone's canonical positions, uploaded by the organiser into `seat_positions` (`F-021` plan §5.3). Issuance accepts only a position on the list, matched exactly, and refuses anything else before it signs or submits (`REQ-ID-3`): `C-14` on the list does not admit `c14`. A position reaches the ledger as its designation's UTF-8 bytes (`positionOf`). Removing a zone removes its positions.
