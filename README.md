...
- **Restriction removal** (`events.tickets.removeRestriction`; `REQ-TK-6`, `AC-B3.4`) frees a ticket of the organiser's event for transfer or resale, signed with their authority. It only ever removes a flag (`INV-10`); removing `cannotResale` from a ticket that cannot be transferred removes both (`REQ-TK-2`). Nothing in Kippu adds a restriction after issuance.
...
- **Scheduled `Finished`** (`events.scheduleFinish`, `events.cancelScheduledFinish`, `events.finishSchedule`; `REQ-EV-12`). Off unless the organiser sets a time; a notice falls due 24 hours before (`noticedAt`: V0 has no mail sender, so the notice is recorded for Ibento to show); cancellable until it runs. The server's finish scheduler runs due notices and finishes every 30 seconds. **The system principal**: a write with no user behind it is audited with `principal_kind` `system`, attributed to the organiser who scheduled it (`organiser_id`), with request id `scheduled-finish:<schedule id>` (`NFR-7`). A ledger refusal when it runs is recorded on the schedule. **Reconciliation** (`T-021-17`): the scheduler runs a pass as soon as the server starts, and each pass first settles schedules left `running` that no live process holds (a running finish holds a per-schedule advisory lock). If the ledger has the event `Finished`, the schedule completes. If the run's submission was refused, it is recorded as refused, and sales the run closed are reopened, as on the live path. If a submission is still unanswered within its operation lifetime plus a minute, it is left for a later pass. Otherwise it runs again. A submission that loses its verdict stays `running` for reconciliation to settle, so nothing is submitted twice while the first submission can still land.
